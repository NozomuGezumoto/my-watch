...
import { getMyWatches, removeMyWatch, type MyWatchEntry } from "../myWatches";
import styles from "./MyWatchesView.module.css";

export function MyWatchesView({ seed }: { seed: SeedData }) {
  const [watches, setWatches] = useState<MyWatchEntry[]>([]);

  useEffect(() => {
    setWatches(getMyWatches());
  }, []);
  ...
  return (
    <div className={styles.page}>
      <h1 className={styles.title}>マイ時計</h1>
      <p className={styles.lead}>
        あなたが持っている時計をコレクション・進化点・バリエーションに紐づけておけます。
      </p>
