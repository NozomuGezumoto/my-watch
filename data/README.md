...
**ヒーローセクション**（コレクション詳細の「この世代のポイント」＝ `whyItMatters` / `heroSubline`）は **現状で固定** とする。  
明確な指示があるまで、**全モデル共通**でヒーロー内容の一括変更・上書きは行わないこと。  
（`align_release_granularity.js` での whyItMatters/heroSubline の書き換え、`dedupe_why_it_matters.js` の再実行も避ける。）
